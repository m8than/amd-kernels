// gemm_a8wfp4: INT8 activations x FP4 weights GEMM
// C = dequant(A_int8) @ dequant(B_fp4)^T
// A (M, K) int8 -> cast to bf16 by host
// B (K/2, N) uint8 (FP4 packed) -> pre-dequantized to bf16 by host
// a_scales (M,) float32 -- per-row scale for A (e8m0 format originally)
// b_scales (N, K/32) float32 -- per-block scale for B (e8m0 format originally)
// C (M, N) bf16
//
// Strategy: Both A and B are pre-dequantized to bf16 by the host.
// Kernel performs standard bf16 MMA, then applies A's per-row scales.
// B scales are folded into the pre-dequantized values by the host.
...
struct gemm_a8wfp4_globals {
    _gl_A a;           // A (pre-cast to bf16 by host)
    _gl_B b;           // B (pre-dequantized to bf16 by host)
...
void gemm_a8wfp4_kernel(const gemm_a8wfp4_globals g, int M, int N, int K) {
    ...
    // Apply per-row A scale after full K accumulation
...
void dispatch_gemm_a8wfp4(gemm_a8wfp4_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)gemm_a8wfp4_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    gemm_a8wfp4_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M, g.N, g.K);
}

PYBIND11_MODULE(gemm_a8wfp4_tk, m) {
    m.doc() = "HipKittens INT8 x FP4 GEMM";
    kittens::py::bind_function<dispatch_gemm_a8wfp4>(m, "dispatch",
        &gemm_a8wfp4_globals::a, &gemm_a8wfp4_globals::b,
        &gemm_a8wfp4_globals::a_scale, &gemm_a8wfp4_globals::c);
}
