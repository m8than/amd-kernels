// gemm_afp4wfp4: FP4 activations x FP4 weights GEMM
// C = dequant(A_fp4) @ dequant(B_fp4)^T
// A (M, K/2) uint8 (FP4 packed) -> pre-dequantized to bf16 by host
// B (K/2, N) uint8 (FP4 packed) -> pre-dequantized to bf16 by host
// a_scales (M, K/32) float32 (e8m0 format, 32 elements share one scale)
// b_scales (N, K/32) float32 (e8m0 format, 32 elements share one scale)
// C (M, N) bf16
//
// Strategy: Both A and B are pre-dequantized to bf16 by the host.
// This is the most complex quantization variant but with host-side dequant
// it reduces to a standard bf16 MMA pipeline.
// Scales are folded into the pre-dequantized values by the host.
...
struct gemm_afp4wfp4_globals {
    _gl_A a;    // A (pre-dequantized to bf16 by host)
...
void gemm_afp4wfp4_kernel(const gemm_afp4wfp4_globals g, int M, int N, int K) {
    ...
    // Store result (scales already folded into pre-dequantized values)
    store(g.c, C_accum, {0, 0,
        pid_m * WARPS_M + warp_row,
        pid_n * WARPS_N + warp_col});
}

void dispatch_gemm_afp4wfp4(gemm_afp4wfp4_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)gemm_afp4wfp4_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    gemm_afp4wfp4_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M, g.N, g.K);
}

PYBIND11_MODULE(gemm_afp4wfp4_tk, m) {
    m.doc() = "HipKittens FP4 x FP4 GEMM";
    kittens::py::bind_function<dispatch_gemm_afp4wfp4>(m, "dispatch",
        &gemm_afp4wfp4_globals::a, &gemm_afp4wfp4_globals::b,
        &gemm_afp4wfp4_globals::c);
}
