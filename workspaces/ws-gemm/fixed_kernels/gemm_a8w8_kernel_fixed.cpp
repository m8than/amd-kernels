// gemm_a8w8: INT8 GEMM with per-tensor dequantization
// C = (A_int8 @ B_int8^T) * a_scale * b_scale
// A (M, K) int8 -> dequant to bf16, B (N, K) int8 -> dequant to bf16
// a_scale (M,) float32, b_scale (N,) float32
// C (M, N) bf16
//
// Strategy: Load int8 tiles, cast to bf16, perform bf16 MMA, apply scales after.
...
struct gemm_a8w8_globals {
    _gl_A a;         // A matrix (already cast to bf16 by host)
    _gl_B b;         // B matrix (already cast to bf16 by host)
    _gl_scale a_scale; // Per-row scale for A: (M,)
    _gl_scale b_scale; // Per-col scale for B: (N,)
    _gl_C c;
    hipStream_t stream;
    int M;
    int N;
...
void gemm_a8w8_kernel(const gemm_a8w8_globals g, int M, int N, int K) {
    ...
    // C[i][j] = accum[i][j] * a_scale[i] * b_scale[j]
    // Load scales into register vectors
    decltype(C_accum)::col_vec a_scale_vec;
    decltype(C_accum)::row_vec b_scale_vec;

    // Load a_scale for this warp's rows
    load(a_scale_vec, g.a_scale, {0, 0, 0, pid_m * BLOCK_M + warp_row * REG_BLOCK_M});
    // Load b_scale for this warp's cols
    load(b_scale_vec, g.b_scale, {0, 0, 0, pid_n * BLOCK_N + warp_col * REG_BLOCK_N});
    asm volatile("s_waitcnt vmcnt(0)");
...
void dispatch_gemm_a8w8(gemm_a8w8_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)gemm_a8w8_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    gemm_a8w8_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M, g.N, g.K);
}

PYBIND11_MODULE(gemm_a8w8_tk, m) {
    m.doc() = "HipKittens INT8 GEMM with per-tensor dequant";
    kittens::py::bind_function<dispatch_gemm_a8w8>(m, "dispatch",
        &gemm_a8w8_globals::a, &gemm_a8w8_globals::b,
        &gemm_a8w8_globals::a_scale, &gemm_a8w8_globals::b_scale,
        &gemm_a8w8_globals::c);
}
