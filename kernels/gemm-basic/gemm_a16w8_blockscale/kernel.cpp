// gemm_a16w8_blockscale: Mixed precision GEMM - FP16 activations x INT8 weights
// C = A_bf16 @ (B_int8 * b_scale)^T
// A (M, K) bf16, B (N, K) int8 -> dequant to bf16 using block scales
// b_scale (scale_k, scale_n) float32
// C (M, N) bf16
//
// For each K-block: load A as bf16, load B as int8->bf16, apply B block scale,
// then MMA. Unlike a8w8_blockscale, A is already in bf16 so no A scale needed
// (unless PREQUANT mode which quantizes A on-the-fly, not implemented here).
...
using _gl_B = gl<bf16, -1, -1, -1, -1>;  // B pre-cast to bf16 by host
using _gl_C = gl<bf16, -1, -1, -1, -1>;
using _gl_scale = gl<float, -1, -1, -1, -1>;

using G = kittens::group<NUM_WARPS>;

struct gemm_a16w8_bs_globals {
    _gl_A a;
    _gl_B b;
    _gl_C c;
...
void gemm_a16w8_blockscale_kernel(const gemm_a16w8_bs_globals g, int M, int N, int K) {
    ...
    for (int kt = 0; kt < num_tiles; kt++) {
        ...
        // Apply B block scale only (A is already bf16, no scale needed)
...
void dispatch_gemm_a16w8_blockscale(gemm_a16w8_bs_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)gemm_a16w8_blockscale_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    gemm_a16w8_blockscale_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M, g.N, g.K);
}

PYBIND11_MODULE(gemm_a16w8_blockscale_tk, m) {
    m.doc() = "HipKittens mixed-precision BF16xINT8 GEMM with block scaling";
    kittens::py::bind_function<dispatch_gemm_a16w8_blockscale>(m, "dispatch",
        &gemm_a16w8_bs_globals::a, &gemm_a16w8_bs_globals::b,
        &gemm_a16w8_bs_globals::c, &gemm_a16w8_bs_globals::b_scale);
}
