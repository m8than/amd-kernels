// gemm_a16wfp4: BF16 activations x FP4 weights GEMM
// C = A_bf16 @ dequant(B_fp4)^T
// A (M, K) bf16
// B (K/2, N) uint8 (FP4 packed: 2 FP4 values per byte)
// b_scales (N, K/32) uint8 (e8m0 format, 32 elements share one scale)
// C (M, N) bf16
//
// Strategy: Dequantize FP4 weights to bf16 on load, then standard bf16 MMA.
...
constexpr int BLOCK_K     = 64;   // In bf16 element space
constexpr int WARPS_M     = 2;
constexpr int WARPS_N     = 4;
constexpr int REG_BLOCK_M = BLOCK_M / WARPS_M;
constexpr int REG_BLOCK_N = BLOCK_N / WARPS_N;
constexpr int SCALE_GROUP  = 32;  // 32 elements share one scale

#define NUM_WARPS (WARPS_M * WARPS_N)
#define NUM_THREADS (kittens::WARP_THREADS * NUM_WARPS)

using _gl_A = gl<bf16, -1, -1, -1, -1>;
using _gl_B = gl<bf16, -1, -1, -1, -1>;  // B pre-dequantized to bf16 by host
using _gl_C = gl<bf16, -1, -1, -1, -1>;
using _gl_scale = gl<float, -1, -1, -1, -1>;

using G = kittens::group<NUM_WARPS>;

struct gemm_a16wfp4_globals {
    _gl_A a;
    _gl_B b;       // Pre-dequantized B in bf16
    _gl_C c;
    _gl_scale b_scales;  // B scales for block dequant
    hipStream_t stream;
    int M;
    int N;
    int K;
...
void gemm_a16wfp4_kernel(const gemm_a16wfp4_globals g, int M, int N, int K) {
...
void dispatch_gemm_a16wfp4(gemm_a16wfp4_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)gemm_a16wfp4_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    gemm_a16wfp4_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M, g.N, g.K);
}

PYBIND11_MODULE(gemm_a16wfp4_tk, m) {
    m.doc() = "HipKittens BF16 x FP4 GEMM";
    kittens::py::bind_function<dispatch_gemm_a16wfp4>(m, "dispatch",
        &gemm_a16wfp4_globals::a, &gemm_a16wfp4_globals::b,
        &gemm_a16wfp4_globals::c, &gemm_a16wfp4_globals::b_scales);
}
