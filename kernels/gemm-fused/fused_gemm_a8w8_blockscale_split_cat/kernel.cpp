// fused_gemm_a8w8_blockscale_split_cat: INT8 blockscale GEMM with split-concatenate epilogue
//
// GEMM: result = blockscale_gemm(A, B, a_scale, b_scale)
//   result shape: (M, N) where N = D * (S1 + S2)
//
// Split-cat epilogue:
//   c1[:, d, :S1] = result columns corresponding to S1 per D-group
//   c2[:, d, :S2] = result columns corresponding to S2 per D-group
//   c1[:, d, S1:S1+S3] = y[:, d, :S3]  (concatenation)
//
// Ported from: reference/triton/fused/fused_gemm_a8w8_blockscale_split_cat.py
...
struct fused_gemm_a8w8_bs_split_cat_globals {
    _gl_bf16 a;           // A (M, K) int8->bf16
    _gl_bf16 b;           // B (N, K) int8->bf16
    _gl_f32  a_scale;     // (M, K/GROUP_K) float32
    _gl_f32  b_scale;     // (K/GROUP_K, N) float32
...
void fused_gemm_a8w8_bs_split_cat_kernel(const fused_gemm_a8w8_bs_split_cat_globals g,
                                           int M, int N, int K,
    ...
    // The GEMM result C_accum has shape (BLOCK_M, BLOCK_N) covering
    // a sub-block of the full (M, N) output where N = D*(S1+S2).
    //
    // For each element (m, n) in the block:
    //   d = n / (S1 + S2)
    //   s = n % (S1 + S2)
    //   if s < S1: write to c1[m, d, s]
    //   else:      write to c2[m, d, s - S1]
    //
    // Then load y[m, d, :S3] and write to c1[m, d, S1:S1+S3]
    //
    // This is done element-wise via the warp's register tile.
    // Since we can't do per-element scatter from register tiles directly,
    // we store to a temporary shared memory tile, then scatter from shared.
    // For simplicity, we write the full GEMM result to global via c1 store path
    // with masking, following the Triton pattern.

    // Store GEMM result back to global as a simple (M, N) block first,
    // then do the split-cat in a second pass. This matches the Triton
    // non-splitk path behavior using masked stores.

    // We need to convert C_accum from register to shared for scatter write.
    // Use the shared memory tiles (As/Bs no longer needed) as scratch.
    using ST_Out = st<bf16,REG_BLOCK_M, REG_BLOCK_N>;
    ST_Out& scratch = *reinterpret_cast<ST_Out*>(&As[0]);

    // Convert fp32 accum to bf16 and store to shared
    ...
    // Each thread handles a few elements from the shared tile
    ...
    // y has shape (M, D, S3), need to write to c1[:, :, S1:S1+S3]
    for (int e = 0; e < elems_per_thread; e++) {
        int flat_idx = lane_id * elems_per_thread + e;
        int local_m = flat_idx / REG_BLOCK_N;
        int local_n = flat_idx % REG_BLOCK_N;
        ...
        // Map local_n to (d, s3_idx) for y loading
        // We reuse the same thread mapping but now for y's S3 columns
        if (local_n < S3 && global_m < M) {
            int d_idx = n_base / stride_per_s;  // Which D-group this warp handles
            if (d_idx < D) {
                // Load y[global_m, d_idx, local_n]
                ...
                // Write to c1[global_m, d_idx, S1 + local_n]
                bf16* c1_ptr = (bf16*)&g.c1[{0, 0, 0, 0}];
                int c1_stride_m = g.c1.template stride<2>();
...
void dispatch_fused_gemm_a8w8_bs_split_cat(fused_gemm_a8w8_bs_split_cat_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)fused_gemm_a8w8_bs_split_cat_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    fused_gemm_a8w8_bs_split_cat_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(
