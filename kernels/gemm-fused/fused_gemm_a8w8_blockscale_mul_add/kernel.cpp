// fused_gemm_a8w8_blockscale_mul_add: INT8 blockscale GEMM with multiply-add epilogue
//
// C = c_a * (blockscale_gemm(A, B, a_scale, b_scale)) + c_b
//
// The multiply-add is fused inline after the GEMM accumulation,
// avoiding an extra write/read of the intermediate GEMM result.
//
// Ported from: reference/triton/fused/fused_gemm_a8w8_blockscale_mul_add.py
...
struct fused_gemm_a8w8_bs_mul_add_globals {
    _gl_bf16 a;           // A (M, K) int8->bf16
    _gl_bf16 b;           // B (N, K) int8->bf16
    _gl_bf16 c;           // C output (M, N) bf16
    _gl_f32  a_scale;     // (M, K/GROUP_K) float32
    _gl_f32  b_scale;     // (K/GROUP_K, N) float32
    _gl_bf16 c_a;         // Multiply operand (M, N) bf16
    _gl_bf16 c_b;         // Add operand (M, N) bf16
    hipStream_t stream;
    int M, N, K;
...
void fused_gemm_a8w8_bs_mul_add_kernel(const fused_gemm_a8w8_bs_mul_add_globals g,
                                         int M, int N, int K) {
    ...
    // ===== FUSED MUL-ADD EPILOGUE =====
    // result = c_a * C_accum + c_b (FUSE_TYPE=0)
    // Load c_a and c_b tiles for this output block
    rt<float,REG_BLOCK_M, REG_BLOCK_N, col_l, rt_16x16_s> ca_tile;
    rt<float,REG_BLOCK_M, REG_BLOCK_N, col_l, rt_16x16_s> cb_tile;
    load(ca_tile, g.c_a, {0, 0,
        pid_m * WARPS_M + warp_row,
        pid_n * WARPS_N + warp_col});
    load(cb_tile, g.c_b, {0, 0,
        pid_m * WARPS_M + warp_row,
        pid_n * WARPS_N + warp_col});

    // C_accum = c_a * C_accum + c_b
    mul(C_accum, ca_tile, C_accum);
    add(C_accum, C_accum, cb_tile);
...
void dispatch_fused_gemm_a8w8_bs_mul_add(fused_gemm_a8w8_bs_mul_add_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)fused_gemm_a8w8_bs_mul_add_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    fused_gemm_a8w8_bs_mul_add_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(
        g, g.M, g.N, g.K);
}

PYBIND11_MODULE(fused_gemm_a8w8_blockscale_mul_add_tk, m) {
    m.doc() = "HipKittens INT8 blockscale GEMM with fused multiply-add epilogue";
    kittens::py::bind_function<dispatch_fused_gemm_a8w8_bs_mul_add>(m, "dispatch",
        &fused_gemm_a8w8_bs_mul_add_globals::a,
        &fused_gemm_a8w8_bs_mul_add_globals::b,
        &fused_gemm_a8w8_bs_mul_add_globals::c,
        &fused_gemm_a8w8_bs_mul_add_globals::a_scale,
        &fused_gemm_a8w8_bs_mul_add_globals::b_scale,
        &fused_gemm_a8w8_bs_mul_add_globals::c_a,
        &fused_gemm_a8w8_bs_mul_add_globals::c_b);
}
