// Batched INT8 GEMM with per-tensor dequantization
// C[b] = (A_int8[b] @ B_int8[b]^T) * a_scale[b] * b_scale[b]  (+ optional bias[b])
//
// A: (B, M, K) int8 -> pre-cast to bf16 by host
// B: (B, N, K) int8 -> pre-cast to bf16 by host (transposed for mma_ABt)
// a_scale: (B, M) float32 — per-row scale
// b_scale: (B, N) float32 — per-col scale
// C: (B, M, N) bf16
// bias: (B, 1, N) bf16 (optional)
//
// Strategy: A/B are pre-cast to bf16 by host. Kernel does bf16 MMA then applies scales.
// Ported from Triton batched_gemm_a8w8.py + wave1-patterns/gemm_a8w8_kernel.cpp
...
struct batched_gemm_a8w8_globals {
    _gl_bf16  a;       // (1, B, M, K) bf16 (pre-cast from int8)
    _gl_bf16  b;       // (1, B, N, K) bf16 (pre-cast, transposed)
    _gl_scale a_scale; // (1, B, 1, M) float32 — per-row scale
    _gl_scale b_scale; // (1, B, 1, N) float32 — per-col scale
...
void batched_gemm_a8w8_kernel(const batched_gemm_a8w8_globals g,
    ...
    // Apply per-token (row) and per-channel (col) scales:
    // C[i][j] = accum[i][j] * a_scale[batch, i] * b_scale[batch, j]
    decltype(C_accum)::col_vec a_scale_vec;
    decltype(C_accum)::row_vec b_scale_vec;

    // a_scale layout: (1, B, 1, M) -> load M-dim slice for this warp's rows
    load(a_scale_vec, g.a_scale, {0, batch_id, 0, pid_m * BLOCK_M + warp_row * REG_BLOCK_M});
    // b_scale layout: (1, B, 1, N) -> load N-dim slice for this warp's cols
    load(b_scale_vec, g.b_scale, {0, batch_id, 0, pid_n * BLOCK_N + warp_col * REG_BLOCK_N});
    asm volatile("s_waitcnt vmcnt(0)");

    // Apply scales: C = accum * a_scale[:,None] * b_scale[None,:]
    mul_row(C_accum, C_accum, a_scale_vec);
    mul_col(C_accum, C_accum, b_scale_vec);
...
void dispatch_batched_gemm_a8w8(batched_gemm_a8w8_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)batched_gemm_a8w8_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    batched_gemm_a8w8_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(
        g, g.M, g.N, g.K, g.has_bias);
}

PYBIND11_MODULE(batched_gemm_a8w8_tk, m) {
    m.doc() = "HipKittens Batched INT8 GEMM with per-tensor dequant";
    kittens::py::bind_function<dispatch_batched_gemm_a8w8>(m, "dispatch",
        &batched_gemm_a8w8_globals::a,
        &batched_gemm_a8w8_globals::b,
        &batched_gemm_a8w8_globals::a_scale,
        &batched_gemm_a8w8_globals::b_scale,
        &batched_gemm_a8w8_globals::c,
        &batched_gemm_a8w8_globals::bias);
}
