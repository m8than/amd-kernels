// Batched FP4 x FP4 GEMM: C[b] = dequant(A_fp4[b]) @ dequant(B_fp4[b])^T
//
// A: (B, M, K/2) uint8 (FP4 packed) -> pre-dequantized to (B, M, K) bf16 by host
// B: (B, K/2, N) uint8 (FP4 packed) -> pre-dequantized to (B, N, K) bf16 by host
// a_scales: (B, M, K/32) (e8m0) -> folded into dequantized A by host
// b_scales: (B, N, K/32) (e8m0) -> folded into dequantized B by host
// C: (B, M, N) bf16
//
// Strategy: Both A and B pre-dequantized to bf16 with scales folded in by host.
// Kernel performs standard bf16 MMA with batch dimension.
// Ported from Triton batched_gemm_afp4wfp4.py + wave1-patterns/gemm_afp4wfp4_kernel.cpp
...
struct batched_gemm_afp4wfp4_globals {
    _gl_bf16 a;    // (1, B, M, K) bf16 (pre-dequantized from FP4)
...
void batched_gemm_afp4wfp4_kernel(const batched_gemm_afp4wfp4_globals g,
                                    int M, int N, int K) {
    ...
    // Store result (scales already folded into pre-dequantized A and B)
    store(g.c, C_accum, {0, batch_id,
        pid_m * WARPS_M + warp_row,
        pid_n * WARPS_N + warp_col});
}

void dispatch_batched_gemm_afp4wfp4(batched_gemm_afp4wfp4_globals g) {
    unsigned long mem_size = g.dynamic_shared_memory();
    hipFuncSetAttribute((void*)batched_gemm_afp4wfp4_kernel,
        hipFuncAttributeMaxDynamicSharedMemorySize, mem_size);
    batched_gemm_afp4wfp4_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(
        g, g.M, g.N, g.K);
}

PYBIND11_MODULE(batched_gemm_afp4wfp4_tk, m) {
    m.doc() = "HipKittens Batched FP4 x FP4 GEMM kernel";
    kittens::py::bind_function<dispatch_batched_gemm_afp4wfp4>(m, "dispatch",
        &batched_gemm_afp4wfp4_globals::a,
        &batched_gemm_afp4wfp4_globals::b,
        &batched_gemm_afp4wfp4_globals::c);
}
