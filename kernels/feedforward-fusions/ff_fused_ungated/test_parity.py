#!/usr/bin/env python3
"""
Parity test for ff_fused_ungated HipKittens kernel

Tests the ungated feed forward kernel against Triton reference.

NOTE: This kernel currently implements stage 1 (first GEMM with activation).
The full Triton kernel also includes stage 2 (second GEMM with W2).
"""
...
def reference_ungated_ff_stage1(X, W1, activation='silu'):
    """
    Reference implementation of stage 1 of ungated feed forward.

    Args:
        X: (M, K) input tensor
        W1: (N, K) weight tensor
        activation: activation function name

    Returns:
        out: (M, N) intermediate output after first GEMM + activation
    """
    M, K = X.shape
    N, K_ = W1.shape
...
def generate_test_inputs(M=256, N=256, K=4096, dtype=torch.bfloat16):
    """Generate random test inputs."""
    X = torch.randn(M, K, dtype=dtype)
    W1 = torch.randn(N, K, dtype=dtype)
    W2 = torch.randn(N, K, dtype=dtype)  # For future stage 2 testing
    return X, W1, W2

def test_parity():
    """Test parity between reference and HipKittens implementation."""
    print("=" * 80)
    print("FF Fused Ungated Kernel Parity Test")
    print("=" * 80)
    ...
    print(f"  Expected output: ({M}, {N}) {dtype}")

    # Generate inputs
    print("\nGenerating test inputs...")
    X, W1, W2 = generate_test_inputs(M, N, K, dtype)

    # Compute reference output (stage 1 only)
    print("Computing reference output (stage 1)...")
    ref_output = reference_ungated_ff_stage1(X, W1, activation='silu')
